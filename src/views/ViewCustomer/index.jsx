...
const ViewCustomer = () => {
  ...
  const fetchBookings = async () => {
    setLoading(true);
    try {
      const res = await getApi(`${urls.booking.customerReport}?customerId=${id}`);
      const bookings = res?.data?.customerBookings || [];
  ...
  return (
    <>
      <CustomBreadcrumbs
        title={t('text.VIEW_CUSTOMER')}
        links={[
          { name: t('text.CUSTOMER'), path: '/customer' },
          { name: t('text.VIEW_CUSTOMER'), path: '' }
        ]}
      />
      <Box>
        <Card sx={{ p: 3, borderRadius: 2 }}>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6}>
              <Typography>
                <strong>{t('text.NAME')}:</strong> {customer.name}
              </Typography>
              <Typography>
                <strong>{t('text.EMAIL')}:</strong> {customer.email}
              </Typography>
              <Typography>
                <strong>{t('text.PHONE')}:</strong> {customer.mobileNo}
              </Typography>
            </Grid>
            ...
            <Grid item xs={12} sm={6}>
              <Typography>
                ...
                <Chip
                  ...
                  size="small"
                  sx={{
                    bgcolor: customer.status === 'Active' ? '#30aa4c' : '#dc3545',
                    color: 'white',
                    fontWeight: 'bold',
                    borderRadius: 1,
                    fontSize: '12px',
              ...
              <Typography>
                <strong>{t('text.ADDRESS')}:</strong> {customer.address}
              </Typography>
            </Grid>
          </Grid>
        </Card>
